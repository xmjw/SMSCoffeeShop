...
post '/forfill/:id' do
  ...
  if order.save
    client.account.messages.create(to: order.customer.number, 
		 ...
		 body: "Your #{order.standard_name} is ready. You can collect it from the coffee shop right away, ask for order number #{order.id}.") if ENV['ENABLE_SMS'] == "YES"
...
post '/cancel/:id' do
  ...
  if order.save
    client.account.messages.create(to: order.customer.number, 
	    from: ENV['TWILIO_NUMBER'], 
	    body: "We're really sorry, but the Barista cancelled your order. We're going to go and talk to them to make sure everything is okay.") if ENV['ENABLE_SMS'] == "YES"
     remove_order_from_squeezer order
    {"status" => "ok"}.to_json
  else
    {"status" => "nok"}.to_json
...
post '/order' do
  ...
  order = nil
	...
	response.headers['x-order-id'] = order.id if order != nil
